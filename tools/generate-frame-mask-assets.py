...
def band(style, variant, segment):
    ...
    rng = seeded("batchframe-v3", style, variant, segment)
    mask = Image.new("L", (width, height), 0)
    draw = ImageDraw.Draw(mask)
    if corner:
        # A continuous corner with a different bend per variant.
        base = 35 + rng.randrange(8)
        for y in range(height):
            for x in range(width):
                edge = min(x, y)
                if edge < base + math.sin((x + y) / 18) * (2 if style == "full-frame-scan" else 5):
                    mask.putpixel((x, y), 255)
    else:
        horizontal = segment in {"top", "bottom"}
        length = width if horizontal else height
        depth = height if horizontal else width
        base = depth * (0.68 if style == "full-frame-scan" else 0.76)
        for i in range(length):
            t = i / max(1, length - 1)
            low = math.sin(t * math.pi * (1.1 + rng.random() * 0.8) + variant) * (2.2 if style == "full-frame-scan" else 5.5)
            if style == "emulsion-damage":
                low += math.sin(t * math.pi * 4.7 + variant * 0.7) * 2.4
            edge_depth = int(max(depth * 0.48, min(depth - 2, base + low)))
            for j in range(edge_depth):
                alpha = 255
                if style == "emulsion-damage" and rng.random() < 0.045:
...
def generate(style, variants):
    for variant in range(1, variants + 1):
        ...
        for segment in SEGMENTS:
            ...
            alpha = orient(band(style, variant, source), segment)
