...
Page({
  ...
  applySecurityResult(result, checkId) {
    ...
    if (index < 0) return;
    this.setData({ [`images[${index}].securityStatus`]: result.status });
  ...
  async ensureSecurityBeforeExport() {
    ...
    if (latest.some(item => item.securityStatus !== 'passed')) {
      wx.showModal({
        title: '暂时无法导出',
  ...
  redrawPreview(){
    if(!this.pCanvas || !this.pCtx || !this.canvasReady){
      this.pendingRender = true;
      return;
    }
